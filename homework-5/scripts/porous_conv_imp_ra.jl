...
@views function porous_convection_implicit_2D(Ra)
    # physics
    lx      = 40.0
    ly      = 20.0
    #λ       = 0.001
    ...
    record(fig, "heatmap_arrows_implicit_$(Ra).gif"; fps=20) do io
        # time loop
        for it in 1:nt
...
#main script
#Run for different Rayleigh numbers

for Ra in [10, 40, 100, 1000]
    @printf("Running simulation for Rayleigh number Ra = %d\n", Ra)
    porous_convection_implicit_2D(Ra)
end
