...
@parallel function compute_flux!(dQx, dQy, Pf,k_ηf_dx,k_ηf_dy,_1_θ_dτ)
...
@parallel function update_Pf!(Pf, dQx, dQy, _dx, _dy, _β_dτ)
    @inn(Pf) = @inn(Pf) - _β_dτ * ( @d_xa(dQx) * _dx + @d_ya(dQy) * _dy )
    return nothing
end


@parallel function compute_residual!(r_Pf, dQx, dQy, _dx, _dy)
    @inn(r_Pf) = @inn(r_Pf) + ( @d_xa(dQx) * _dx + @d_ya(dQy) * _dy )
    return nothing
end


function Pf_diffusion_2D_xpu(;do_viz=false)
...
if isinteractive()
    Pf_diffusion_2D_xpu(;do_viz=false)
end