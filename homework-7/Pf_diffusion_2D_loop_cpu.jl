...
function Pf_diffusion_2D(nx, ny; do_viz=false, save_Pf_at::Union{Nothing,Int}=nothing, save_path::Union{Nothing,String}=nothing)
    # benchmark 
    t_tock = 0.0
    t_start = 0.0
    ...
    while err_Pf >= ϵtol && iter <= maxiter
        ...
        if save_Pf_at !== nothing && iter == save_Pf_at
            Pf_host = Pf  # bring to CPU
            path = isnothing(save_path) ? "Pf_cpu_$(save_Pf_at)_nx$(nx)_ny$(ny).jld2" : save_path
...
Pf_diffusion_2D(nx, ny; do_viz=false, save_Pf_at=60, save_path="Pf_cpu_60.jld2")