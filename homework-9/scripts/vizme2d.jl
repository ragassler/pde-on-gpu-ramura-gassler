...
@views function vizme2D_mpi(nprocs)
    ...
    for ipx = 1:nprocs[1]
        for ipy = 1:nprocs[2]
            file = matopen("scripts/mpi2D_out_C_$(ip-1).mat")
    ...
    savefig("docs/diffusion_2D_mpi.png")
    return
end

## Whole time steps visualization as gif

@views function vizme2D_mpi_gif(nprocs, nt)
    anim = @animate for it = 1:nt
        C = []; ip = 1
        for ipx = 1:nprocs[1]
            for ipy = 1:nprocs[2]
                file = matopen("vis_gpu/mpi2D_out_C_$(ip-1)_$(it).mat")
                C_loc = read(file, "C"); close(file)
                nx_i, ny_i = size(C_loc, 1) - 2, size(C_loc, 2) - 2
                ix1, iy1 = 1 + (ipx - 1) * nx_i, 1 + (ipy - 1) * ny_i
                if (ip == 1) C = zeros(nprocs[1] * nx_i, nprocs[2] * ny_i) end
        ...
        opts = (aspect_ratio=1, yaxis=font(fontsize, "Courier"), xaxis=font(fontsize, "Courier"),
                ticks=nothing, framestyle=:box, titlefontsize=fontsize, titlefont="Courier",
                xlabel="Lx", ylabel="Ly", xlims=(1, size(C, 1)), ylims=(1, size(C, 2)), clim=(0,1))
        heatmap(C'; c=:turbo, title="diffusion 2D MPI (it=$(it))", opts...)
    end
    gif(anim, "docs/diffusion_2D_mpi_gpu.gif", fps=40)
    return
end

vizme2D_mpi_gif(nprocs, 100)