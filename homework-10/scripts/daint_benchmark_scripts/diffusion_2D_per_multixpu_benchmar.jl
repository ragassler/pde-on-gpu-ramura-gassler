...
@views function diffusion_2D(nx, ny; do_visu=false, do_save=false, init_MPI=true, finalize_MPI=true)
    # Physics
    Lx, Ly  = 10.0, 10.0
    D       = 1.0
    ttot    = 1e0
    ...
    me, dims = init_global_grid(nx, ny, 1;
                                select_device=false,
                                init_MPI=init_MPI)   # <-- important  # Initialization of MPI and more...
    ...
    nt      = cld(ttot, dt)
    nt      = min(nt, 1000)  # limit to 1000 iterations for benchmarking
    ...
    for it = 1:nt
        ...
        # @hide_communication (2,2) begin
        @hide_communication (16,4) begin
        # @hide_communication (16,16) begin
    ...
    if do_save
        ...
        gather!(C_inn, C_v)
        if do_save && (me==0)
            file = matopen("$(@__DIR__)/MXPU2D_out_C.mat", "w"); write(file, "C", Array(C_v)); close(file)
        end
    end

    finalize_global_grid(; finalize_MPI=finalize_MPI)  # <-- also important
    return t_toc
end
